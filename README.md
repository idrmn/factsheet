...
agent's PATH. The `SessionStart` hook is part of the plugin: do not add it to
...
Add the hook yourself in `~/.claude/settings.json`:
...
### Hook per project (cargo install)
...
`~/.cursor/hooks/factsheet-context.sh`, executable. Needs `jq`:
...
`~/.gemini/config/factsheet-bootstrap.sh`, executable. Needs `jq`:

```sh
#!/bin/sh
command -v jq >/dev/null || { echo '{"injectSteps": []}'; exit 0; }
input=$(cat)
cid=$(printf '%s' "$input" | jq -r '.conversationId // empty')
stamp="/tmp/factsheet-inject-${cid}"
if [ -n "$cid" ] && [ ! -e "$stamp" ]; then
  touch "$stamp"
  ws=$(printf '%s' "$input" | jq -r '.workspacePaths[0] // empty')
  [ -d "$ws" ] && cd "$ws"
  ctx=$(factsheet 2>/dev/null)
