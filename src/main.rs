...
split into atomic facts, prefix with topic (`azure db: ...`), move commands to -c"
    ...
    // The curate sheet uses these markers to split text from check.
    ensure!(
        !text.starts_with("check: ") && !text.contains(" | check: "),
        "fact text cannot start with `check: ` or contain ` | check: `; put the command in -c"
    );
    Ok(())
}
